...
searchInput.addEventListener("input", () => {
   ...
   const includes = pokedexIndex.filter((p) => !p.name.startsWith(q) && p.name.includes(q));
...
async function loadPokemon(nameOrId) {
   try {
      ...
      infoEl.textContent = flavorEn ? flavorEn.flavor_text.replace(/\f|\n/g, " ") : "—";
      ...
      abilitiesEl.textContent = pokemon.abilities.map((a) => `${capitalize(a.ability.name)}${a.is_hidden ? " (Hidden)" : ""}`).join(", ");

      // Generation (e.g., "generation-i")
      generationsEl.textContent = species.generation ? prettyGen(species.generation.name) : "—";

      // Weaknesses (derive from type damage relations; union weaknesses)
      const weaknesses = await computeWeaknesses(typeNames);
      weaknessesEl.textContent = weaknesses.length ? weaknesses.map(capitalize).join(", ") : "—";
...
async function computeWeaknesses(typeNames) {
   ...
   const typeData = await Promise.all(typeNames.map((t) => getJSON(`${API_BASE}/type/${t}`)));
   // Collect damage relations: double_damage_from indicates weaknesses
   const weakSets = typeData.map((td) => td.damage_relations.double_damage_from.map((x) => x.name));
   // Merge/unique
...
shinyBtn.addEventListener("click", () => {
   ...
   const nextSrc = isShiny ? imgPokemon.dataset.default : imgPokemon.dataset.shiny;
   if (nextSrc) {
